...
takeFirstList(0, _, []).
takeFirstList(Index, [Y|Ys], Zs):- Ind1 is Index-1, takeFirstList(Ind1, Ys, Ks), append([Y], Ks, Zs).
...
noattack
create(Num,L):- create_list(Num,[],L).
...
/*Da modificare con la subst*/

mutation(X, Xs):-
    random(1,5, Num), 
    random(1,5, Index),
    length(X, Length),
    LengthPrev is Index-1,
    LengthNext is Length-Index,
    takeFirstList(LengthPrev, X, Ys),
    reverse(X, Hs),
    takeFirstList(LengthNext, Hs, Ts),
    reverse(Ts, Zs),
    append(Ys, [Num], Rs),
    append(Rs, Zs, Xs).
...
start(N, Solution):-		%per scacchiera NxN, cambiare Population con N
    create(N, Q),
    reverse(Q, Qreverse),
    permutation(Q, Q1),
 ...
 /*
    permutation([1,2,3,4], Q1),
    reverse([1,2,3,4], Help),
    permutation(Help, Q2),
   */ 
